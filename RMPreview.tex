...
The last possibility we will describe is that the merging black holes may not have formed in \ilya{the same} binary at all.  Instead they formed from the collapse of independent massive stars \ajf{(perhaps each in their own separate binary)}, but were then introduced to each other by a matchmaker, or rather a whole array of matchmakers.   This evolution is illustrated in \autoref{fig:dynamical}: 
\begin{enumerate}
...
\item[c,d.] Subsequent interactions with other objects in the cluster -- the matchmakers -- will gradually tighten the black-hole binary: the interlopers are likely to leave with slightly higher \ajf{speeds} than they arrived with, each time removing energy from the binary.   
